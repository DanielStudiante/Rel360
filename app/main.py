...
async def upload_file(
    ...
    try:
        ...
        return {"message": "Archivo subido correctamente", "file_id": file_id, "duplicate": False}
...
async def extract_text(
    ...
    try:
        ...
        return {"success": True, "cached": False, "file_id": payload.file_id, "extracted_text": text}
...
async def get_nutrition_table(
    payload: NutritionTableRequest,
    current_user: dict = Depends(get_current_user),
):
    try:
    ...
    try:
        ...
        if not extracted:
            raise HTTPException(status_code=400, detail="Primero debes llamar a /extract-text")
